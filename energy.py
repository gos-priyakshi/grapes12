...
def energy_s(args, gcn_gf, gcn_c, val_loader, node_map, data, adjacency, num_indicators, layer_nums, device):
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        x = data.x[all_nodes].to(device)
        intermediate_outputs = gcn_c.get_intermediate_outputs(x, local_edge_indices)
        # check intermediate outputs shape
